...
app.get('/audio', (req, res) => {
    const query = `SELECT * FROM audio ORDER BY id DESC`;
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.json(results);
            console.log("音频伪造检测数据加载成功");
        }
    });
});

app.post('/audio/add', (req, res) => {
    const { id, filename, fileUrl, fileType, probability, datatime, statue, pdf_path } = req.body;
    const sqlparm = [ id, filename, fileUrl, fileType, probability, datatime, statue, pdf_path];
    const query = `INSERT INTO audio (uuid, fileName,fileUrl, fileType, probability, datetime, status, pdf_path) VALUES (?,?,?,?,?,?,?,?)`;
...
app.delete('/audio/:id', async (req, res) => {
    ...
    const query1 = `SELECT fileUrl,pdf_path from audio where uuid = ?`;
    const query2 = `DELETE FROM audio WHERE uuid = ?`;
...
// 监听端口
app.listen(port, () => {
    console.log(`get_audio_db running on port ${port}`);
});
