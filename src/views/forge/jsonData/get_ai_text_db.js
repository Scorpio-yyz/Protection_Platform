...
// 获取所有样本，按 id 字段倒序排序
app.get('/ai_text', (req, res) => {
    const query = `SELECT * FROM ai_text ORDER BY id DESC`;
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.json(results);
            console.log("AIGC伪造检测数据加载成功");
        }
    });
});

// 删除指定 id 的数据
app.delete('/ai_text/:id', async (req, res) => {
    const id = req.params.id;
    const query1 = `SELECT fileUrl,pdf_path from ai_text where uuid = ?`;
    const query2 = `DELETE FROM ai_text WHERE uuid = ?`;
...
// 添加文件的函数
app.post('/ai_text/add', (req, res) => {
    const { id, fileType, probability, describe, datatime, statue, pdf_path } = req.body;
    const sqlparm = [ id, describe, fileType, probability, datatime, statue, pdf_path];
    const query = `INSERT INTO ai_text (uuid, describe, fileType, probability, datetime, status, pdf_path) VALUES (?,?,?,?,?,?,?)`;
    db.query(query, sqlparm, (err, result) => {
        if (err) {
...
// 监听端口
app.listen(port, () => {
    console.log(`get_ai_text_db running on port ${port}`);
});
