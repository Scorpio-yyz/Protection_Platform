...
// 获取所有样本，按 id 字段倒序排序
app.get('/document_img', (req, res) => {
    const query = `SELECT * FROM document_img ORDER BY id DESC`;
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.json(results);
            console.log("图像伪造检测数据加载成功");
        }
    });
});

// 删除指定 id 的数据
app.delete('/document_img/:id', async (req, res) => {
    const id = req.params.id;
    const query1 = `SELECT fileUrl,pdf_path from document_img where uuid = ?`;
    const query2 = `DELETE FROM document_img WHERE uuid = ?`;
...
// 监听端口
app.listen(port, () => {
    console.log(`get_document_image_db running on port ${port}`);
});
