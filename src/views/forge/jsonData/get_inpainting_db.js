...
// 获取所有样本，按 id 字段倒序排序
app.get('/inpainting_mask_img', (req, res) => {
    console.log('hhhhh');
    const query = `SELECT * FROM inpainting_mask_img ORDER BY id DESC`;
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.json(results);
            console.log("待修复的图片数据加载成功");
        }
    });
});

// 删除指定 id 的数据
app.delete('/inpainting_mask_img/:id', async (req, res) => {
    const id = req.params.id;
    const query1 = `SELECT fileUrl,pdf_path from inpainting_mask_img where uuid = ?`;
...
// 监听端口
app.listen(port, () => {
    console.log(`get_inpainting_db running on port ${port}`);
});
