...
// 获取所有样本，按 id 字段倒序排序
app.get('/inpainting_detection_img', (req, res) => {
    console.log('hhhhh');
    const query = `SELECT * FROM inpainting_detection_img ORDER BY id DESC`;
    db.query(query, (err, results) => {
        if (err) {
            ...
            res.json(results);
            console.log("待检测修复的图片数据加载成功");
        }
    });
});

// 删除指定 id 的数据
app.delete('/inpainting_detection_img/:id', async (req, res) => {
    const id = req.params.id;
    const query1 = `SELECT fileUrl,pdf_path from inpainting_detection_img where uuid = ?`;
    const query2 = `DELETE FROM text_img WHERE uuid = ?`;
...
// 监听端口
app.listen(port, () => {
    console.log(`get_inpainting_detection_db running on port ${port}`);
});
