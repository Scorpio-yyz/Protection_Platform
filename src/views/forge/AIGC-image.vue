<template>
    ...
    <AddForgeDialog v-model:visible="state.addMissionVisible" fileType="AI图片" @initialData="addDetect" @success="updateProbability"
        :status="state.detailData.status" />
</template>

<script name="ForgeImage" setup>
import { formatStatusColor, downLoadFile } from "@/utils";
import forgeData from "./jsonData/imgDataList.js";
...
const previewImgList = computed(() => {
    return state.tableData.filter(item => item.fileType === 'AI图片').map(item => item.url)
})

// 获取列表
const getList = async() => {
    // try {
    //     // const response = await axios.get('http://49.123.117.252:3001/image'); // 请求后端接口
        const response = await axios.get('http://49.123.114.186:5555/ImageDetect'); // 请求后端接口
    // 映射uuid到id
