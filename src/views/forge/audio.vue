<template>
    ...
    <AddForgeDialog v-model:visible="state.addMissionVisible" fileType="音频" @initialData="addDetect"  @success="updateProbability"
        :status="state.detailData.status" />
</template>
  
<script name="ForgeAudio" setup>
import { formatStatusColor, downLoadFile } from "@/utils";
import forgeData from "./jsonData/audioDataList.js";
import AddForgeDialog from "./components/AddForgeDialog";
import ForgeColBox from "./components/ForgeColBox.vue";
...
const getList = async() => { 
    try {
        const response = await axios.get('http://49.123.117.252:3090/audio'); // 请求后端接口
        // 映射uuid到id
