<template>
    ...
    <AddForgeDialog v-model:visible="state.addMissionVisible" fileType="图文" @initialData="addDetect" @success="updateProbability"
...
const getList = async() => {
    try {
        const response = await axios.get('http://49.123.117.252:3000/text'); // 请求后端接口
        // 映射uuid到id
