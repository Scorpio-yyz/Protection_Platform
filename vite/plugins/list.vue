<template>
    <div class="watermark-container">
        <el-row justify="end">
            <el-button type="primary" @click="addWater">嵌入水印</el-button>
        </el-row>
        <el-table class="tab-con" :data="state.tableData" stripe border style="width: 100%">
            ...
            <el-table-column prop="file2Url" label="水印文件" width="220" align="center">
                <template #default="{ row }">
                    <img v-if="row.fileType === '图片'" class="pre-img" :src="row.file2Url" :alt="row.fileName"
                        @click="preview(row, 'watermark')">
                    <video v-if="row.fileType === '视频'" class="pre-video" :src="row.file2Url"
                        controls="controls"></video>
                    <audio v-if="row.fileType === '音频'" controls>
                        <source :src="row.file2Url" type="audio/mpeg">
                        Your browser does not support the audio element.
                    </audio>
                </template>
            </el-table-column>
    ...
    <AddWaterDialog v-model:visible="state.addVisible" @success="embedSuccess" />
...
const download = async (row) => {
    const fileUrl = row.file2Url;
    console.log(row.file2Url);
...
import AddWaterDialog from "./components/AddWaterDialog.vue";
import resouceJson from "./jsonData/resouceList.js"
import { convertFileSize } from "@/utils/index";
import { computed } from "vue";

const searchOpt = [
    {
        label: '水印信息', prop: 'waterName', placeholder: '请输入水印信息', required: false, type: 'input'
    },
    {
